...
class WandBHandler(KVWriter):
    """
    Dumps key/value pairs into TensorBoard's numeric format.
    """
    def __init__(self, project, group, name, job_type, cfg, wandb_dir) -> None:
        wandb.init(project=project,group=group, name=name, job_type=job_type, config=cfg, dir=wandb_dir)
        super().__init__()
        ...
        for file_name, fmt in output_config.items():
            try:
                if file_name != 'wandb':
                    ...
                    self._output_handlers.append(WandBHandler(project=fmt['project'],group=fmt['group'],name=fmt['name'],job_type=fmt['job_type'],cfg=fmt['cfg'],wandb_dir=fmt['wandb_dir']))
            except KeyError:
...
def make_log_dirs(
    ...
    log_dirs = os.path.join(save_dir, task_name, algo_name, exp_name)
