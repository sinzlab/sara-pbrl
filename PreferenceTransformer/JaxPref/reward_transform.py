...
def load_queries_with_indices(env, dataset, num_query, len_query, label_type, saved_indices, saved_labels, balance=False, scripted_teacher=False):
    ...
    assert num_query<=len(saved_labels) 
    query_range = np.arange(num_query) #for num_query less than saved_labels, then this query_range commented out would create queries different from the videos in human_label_preprocess.py
    ...
    batch['next_observations'] = seg_next_obs_1
    batch['actions'] = seq_act_1
    batch['rewards']=seg_reward_1
    batch['observations_2'] = seg_obs_2
    batch['next_observations_2'] = seg_next_obs_2
    batch['actions_2'] = seq_act_2
    batch['rewards_2']=seg_reward_2
    batch['timestep_1'] = seq_timestep_1
    batch['timestep_2'] = seq_timestep_2
    batch['start_indices'] = saved_indices[0][query_range]
    batch['start_indices_2'] = saved_indices[1][query_range]
